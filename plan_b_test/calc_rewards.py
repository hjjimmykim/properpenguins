...
def calc_rewards(t, s, term):
    ...
    reward_eligible_mask = term.view(batch_size).clone().byte() # 128
    if reward_eligible_mask.max() == 0:
        # if none of them accepted proposal, by terminating
        return rewards_batch
    exceeded_pool, _ = ((s.last_proposal - s.pool) > 0).max(1) #exceeded_pool = [1,0,0,1,1]
    ...
    proposer = 1 - agent
    accepter = agent
    proposal = torch.zeros(batch_size, 2, 3).long() #item each agent get
    proposal[:, proposer] = s.last_proposal
    proposal[:, accepter] = s.pool - s.last_proposal
    max_utility, _ = s.utilities.max(1) #max(agent1, agent2) 128*3
    ...
    for b in reward_eligible_idxes:
        ...
        available_prosocial = max_utility[b].cpu().dot(s.pool[b].cpu()) #possible maximum prosocial reward
        if available_prosocial != 0:

            scaled_rewards[2] = actual_prosocial.type(torch.FloatTensor) / available_prosocial.type(torch.FloatTensor)

        for i in range(2):
            max_agent = s.utilities[b, i].cpu().dot(s.pool[b].cpu())
            if max_agent != 0:
                scaled_rewards[i] = raw_rewards[i].type(torch.FloatTensor) / max_agent.type(torch.FloatTensor)
