...
class NumberSequenceEncoder2(nn.Module):
    def __init__(self, num_values, num_sources = 3, embedding_size=100):
        """
        eg for values 0,1,2,3,4,5, num_values will be: 6
        for 0,1,..,9 num_values will be: 10
        """
...
#        x = x.transpose(0, 1)
        #x: 3*128
        x = self.embedding(x).view(batch_size, seq_len*self.embedding_size)
        #x: 128*300
        type_constr = torch.cuda if x.is_cuda else torch
        x = self.h1(x)
        x = F.relu(x)
...
class CombinedNet(nn.Module):
    def __init__(self, num_sources=4, embedding_size=100):
        super().__init__()
        self.embedding_size = embedding_size
        self.h1 = nn.Linear(embedding_size * num_sources, embedding_size)
...
class AgentModel(nn.Module):
    def __init__(
        ...
        self.pool_net = NumberSequenceEncoder2(num_values=6)
        self.utility_net = NumberSequenceEncoder2(num_values=11)
        self.utterance_net = NumberSequenceEncoder(num_values=10)
        self.proposal_net = NumberSequenceEncoder2(num_values=6)
        self.proposal_net.embedding = self.pool_net.embedding
...
#        context = torch.cat([pool, utility], 1) #128*6
        pool_h = self.pool_net(pool) #128*100
        util_h = self.utility_net(utility)
        ...
        h_t = torch.cat([pool_h, util_h, m_h, p_h], -1) #128*400
