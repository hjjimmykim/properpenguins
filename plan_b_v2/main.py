...
for epoch in range(200000):
    ...
    #     print('test reward=%.3f' % (test_rewards_sum / len(test_batches)))
        ...
        np.savetxt("./test_rewards_TTT_%.4f%.4f%.4f_sum.csv"%(term_entropy_reg, utterance_entropy_reg,proposal_entropy_reg), np.array(t_reward))
        np.savetxt("./test_rewards_TTT_%.4f%.4f%.4f_sum1.csv"%(term_entropy_reg, utterance_entropy_reg,proposal_entropy_reg), np.array(t_reward1))
        np.savetxt("./test_rewards_TTT_%.4f%.4f%.4f_sum2.csv"%(term_entropy_reg, utterance_entropy_reg,proposal_entropy_reg), np.array(t_reward2))
        for a in range(len(agent_models)):
            torch.save(agent_models[a].state_dict(), "./saved_models_TTT_%.4f%.4f%.4f_agent%i"%(term_entropy_reg, utterance_entropy_reg,proposal_entropy_reg,a)) 
